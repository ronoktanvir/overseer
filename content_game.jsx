...
export class ContentGame extends React.Component {
    ...
    updateDeadlineTimer() {
        ...
        clearInterval(this._overseerInterval);
        clearInterval(this._overseerInterval);
        }
    ...
    render() {
        ...
        const navigation = [
            ...
            ['Overseer dashboard', () => this.setState({overseerVisible: !this.state.overseerVisible})],
            ...
            <main>
                ...
                {this.state.overseerVisible && (
                    <div style={{
                        ...
                        boxShadow:'-10px 0 25px rgba(15,23,42,0.25)',
                        fontFamily:'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
                    }}>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'8px'}}>
                            <div style={{display:'flex',flexDirection:'column'}}>
                                <span style={{fontSize:'11px',letterSpacing:'0.18em',textTransform:'uppercase',color:'#6b7280'}}>Overseer</span>
                                <span style={{fontSize:'13px',fontWeight:600,color:'#111827'}}>Coalition dashboard</span>
                            </div>
                            <button
                                type="button"
                                onClick={() => this.setState({overseerVisible: false})}
                                style={{border:'none',background:'transparent',color:'#9ca3af',cursor:'pointer',fontSize:'16px',padding:'2px 4px',lineHeight:1}}>
                                ×
                            </button>
                        </div>
                        {this.state.overseer ? (
                            <div>
                                <div style={{marginBottom:'12px',fontSize:'11px',color:'#4b5563'}}>
                                    <div style={{marginBottom:'4px',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.12em'}}>Coalition stability</div>
                                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                                        <span style={{fontSize:'13px',fontWeight:600,color:this.state.overseer.reward ? '#15803d' : '#b91c1c'}}>
                                            {this.state.overseer.reward ? 'Stable' : 'Fragile'}
                                        </span>
                                        <span style={{fontSize:'11px',color:'#6b7280'}}>
                                            Window {this.state.overseer.reward_history ? this.state.overseer.reward_history.length : 0}
                                        </span>
                                    </div>
                                </div>
                                <div style={{marginBottom:'12px'}}>
                                    <div style={{fontSize:'11px',color:'#6b7280',marginBottom:'4px'}}>Recent rewards</div>
                                    <div style={{display:'flex',gap:'2px'}}>
                                        {this.state.overseer.reward_history && this.state.overseer.reward_history.map((r,i) => (
                                            <div key={i} style={{
                                                flex:1,
                                                height:'8px',
                                                borderRadius:'999px',
                                                background: r ? '#22c55e' : '#f97316'
                                            }}/>
                                        ))}
                                    </div>
                                </div>
                                <div>
                                    <div style={{fontSize:'11px',color:'#6b7280',marginBottom:'4px',textTransform:'uppercase',letterSpacing:'0.12em'}}>Agents</div>
                                    {this.state.overseer.predictions && Object.entries(this.state.overseer.predictions).map(([p,v]) => {
                                        const lastTurn = this.state.overseer.last_turn || {};
                                        const correct = lastTurn.correct && lastTurn.correct.hasOwnProperty(p) ? lastTurn.correct[p] : null;
                                        const mark = correct === null ? '' : (correct ? '✓' : '✗');
                                        const markColor = correct === null ? '#9ca3af' : (correct ? '#15803d' : '#b91c1c');
                                        return (
                                            <div key={p} style={{marginBottom:'8px',padding:'6px 8px',borderRadius:'8px',background:'#ffffff',border:'1px solid #e5e7eb'}}>
                                                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'2px'}}>
                                                    <span style={{fontSize:'11px',fontWeight:600,color:'#111827'}}>{p}</span>
                                                    {mark && (
                                                        <span style={{fontSize:'11px',fontWeight:600,color:markColor}}>
                                                            {mark}
                                                        </span>
                                                    )}
                                                </div>
                                                <div style={{fontSize:'11px',color:'#4b5563'}}>{v}</div>
                                            </div>
                                        );
                                    })}
                                </div>
                            ...
                            <div style={{color:'#6b7280',fontSize:'12px'}}>Connecting to overseer…</div>
                        )}
                    </div>
                )}
            </main>
    ...
    componentDidMount() {
        window.scrollTo(0, 0);
        const fetchOverseer = () => fetch("http://localhost:8000/overseer_state").then(r => r.json()).then(data => this.setState({overseer: data})).catch(() => {});
        fetchOverseer();
        this._overseerInterval = setInterval(fetchOverseer, 2000);
        if (this.props.data.client)
